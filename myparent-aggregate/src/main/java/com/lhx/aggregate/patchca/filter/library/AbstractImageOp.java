 ...
 */
package com.lhx.aggregate.patchca.filter.library;
...
public abstract class AbstractImageOp implements BufferedImageOp {
	...
	private int linear(int from, int to, int shift, double d) {
		return ((int) Math
				.floor(((from >> shift) & 0xff) + d * (((to >> shift) & 0xff)
						- ((from >> shift) & 0xff)))) << shift;
	}
	...
	protected int bilinear(int nw, int ne, int sw, int se, double xd,
			double yd) {
	...
	public BufferedImage createCompatibleDestImage(BufferedImage src,
		...
		return new BufferedImage(destCM,
				destCM.createCompatibleWritableRaster(src.getWidth(),
						src.getHeight()),
